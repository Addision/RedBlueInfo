...
CTimeTask* CTimeTask::getInstance()
{
	if(timeTask == NULL)
...
CTimeTask::CTimeTask()
{
	...
	{  
		fprintf(stderr,  "create timer err\n");  
		return;  
	}  
}
...
int CTimeTask::excuteTask(int msec, func onFunc, int mode)
{
	...
	{  
		fprintf(stderr, "set time task err\n");
		::timer_delete(timeID);  
		return -1;  
	}  
}
...
/************************************************************************
  ...
  输出参数：
    string     - 新生成的日期
************************************************************************/
std::string CDate::DateSpan(std::string strDate, long nSpan, char cDelimiter /* = '-' */)
{
...
************************************************************************/
void CDate::GetCurrentMonth(std::string &strFrom, std::string &strTo)
{
...
/************************************************************************
  ...
  返 回 值：
    int     - 周几(1为周一，7为周日)
************************************************************************/
int CDate::GetWeekDay(std::string strDate)
{
    struct tm day = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    time_t now = time(NULL);
...
************************************************************************/
int CDate::GetWeek(std::string strDate)
{
    std::string strFirstWeek = strDate.substr(0, 4) + "-01-01";
...
/************************************************************************
  ...
  返 回 值：
    int         - 两个日期的差(分钟)
************************************************************************/
long CDate::MinuteDiff(std::string strDateFrom, std::string strDateTo)
{
    struct tm day = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
...
/************************************************************************
  ...
  返 回 值：
    int         - 两个日期的差(天数)
************************************************************************/
long CDate::DateDiff(std::string strDateFrom, std::string strDateTo)
{
    struct tm day = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    time_t from = time(NULL);
    time_t to = time(NULL);
...
/************************************************************************
  ...
  返 回 值：
    int         - 两个日期的差(天数)
************************************************************************/
int CDate::YearDiff(std::string strDateFrom, std::string strDateTo)
{
    struct tm day = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    {
        std::string strTemp = strDateFrom;
...
************************************************************************/
std::string CDate::GetMonthStart(std::string strDate)
{
...
/************************************************************************
  ...
  返 回 值：
    string  - 月起始日期(1980-01-01)
*********************************************************************/
std::string CDate::GetYear(std::string strDate)
{
    std::string strCur = strDate.substr(0, 4);
...
/************************************************************************
  ...
  返 回 值：
    string  - 月起始日期(1980-01-01)
*********************************************************************/
std::string CDate::GetYearMonth(std::string strDate)
{
    int nYear, nMonth;
    std::string strCur;
...
************************************************************************/
std::string CDate::GetMonthEnd(std::string strDate)
{
    char buff[32];
    int nYear, nMonth;
...
std::string CDate::GetWeekStartEnd(int year,int weekorder)
{
	...
	char buff[5];
	sprintf(buff,"%d",year);
	std::string datFDayFirstWeek,datLDayFirstWeek;
	datFDayFirstWeek=std::string(buff)+"-01-01";//当年的第一天
	...
	{
		std::string datFDayTheWeek,datLDayTheWeek;
		...
		else if(weekorder==53)
		{
			...
			{
				sprintf(buff,"%d",year);
				datLDayTheWeek = std::string(buff)+"-12-31";
			}
			return datFDayTheWeek+"|"+datLDayTheWeek;
		}
		else if(weekorder ==54)
		{
			...
			{
				sprintf(buff,"%d",year);
				datFDayTheWeek=datLDayTheWeek=std::string(buff)+"-12-31";
				return datFDayTheWeek+"|"+datLDayTheWeek;            
			}
			else
			{
...
std::string CDate::FilterNumber(const std::string &str)
{
    char buff[256];
